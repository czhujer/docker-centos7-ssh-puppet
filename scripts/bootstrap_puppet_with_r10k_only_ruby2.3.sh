...
# Install Ruby
$sudo rvm install ruby-2.3.5
$sudo rvm alias create default ruby-2.3.5
...
if [[ `$sudo rpm -qa \*-release | grep -Ei "oracle|redhat|centos|openvz" | cut -d"-" -f3`  =~ ^7 ]]; then
  ...
  $sudo rvm 2.3.5 do gem update --system
  $sudo rvm 2.3.5 do gem install json_pure --no-ri --no-rdoc
  $sudo rvm 2.3.5 do gem install facter --no-ri --no-rdoc
  $sudo rvm 2.3.5 do gem install puppet --no-ri --no-rdoc -v4.10.8
  $sudo rvm 2.3.5 do gem install libshadow --no-ri --no-rdoc
  $sudo rvm 2.3.5 do gem install puppet-module --no-ri --no-rdoc
  $sudo rvm 2.3.5 do gem install ruby-augeas --no-ri --no-rdoc
  $sudo rvm 2.3.5 do gem install syck --no-ri --no-rdoc

  # install r10k
  $sudo rvm 2.3.5 do gem install --no-rdoc --no-ri r10k
  ...
  # Update rubygems, and pull down facter and then puppet...
  $sudo rvm 2.3.5 do gem update --system
  $sudo rvm 2.3.5 do gem install json_pure -v1.8.3 --no-ri --no-rdoc
  $sudo rvm 2.3.5 do gem install facter --no-ri --no-rdoc
  $sudo rvm 2.3.5 do gem install puppet --no-ri --no-rdoc -v3.8.7
  $sudo rvm 2.3.5 do gem install libshadow --no-ri --no-rdoc
  $sudo rvm 2.3.5 do gem install puppet-module --no-ri --no-rdoc
  $sudo rvm 2.3.5 do gem install ruby-augeas --no-ri --no-rdoc
  $sudo rvm 2.3.5 do gem install syck --no-ri --no-rdoc
  ...
  #fix puppet
  $sudo sed -e 's/  Syck.module_eval monkeypatch/  #Syck.module_eval monkeypatch/' -i /usr/local/rvm/gems/ruby-2.3.5/gems/puppet-3.8.7/lib/puppet/vendor/safe_yaml/lib/safe_yaml/syck_node_monkeypatch.rb

fi;

#remove old versions
$sudo rvm uninstall 2.3.3
$sudo rvm uninstall 2.3.4
